...
/* enable_irq(irq_num) enables the given IRQ line
...
void enable_irq(uint32_t irq_num) {
    ...
    /* turning the bit corresponding to the IR port off */
    value = inb(port) & ~(1 << irq_num);
    /* write back to the port */    
    outb(value, port);                      
}

/* disable_irq(irq_num) disables the given IRQ line
...
void disable_irq(uint32_t irq_num) {
    ...
    /* turning the bit corresponding to the IR port on */
    value = inb(port) | (1 << irq_num);     
    /* write back to the port */
    outb(value, port);                      
}

/* send_eoi(irq_num) sends end of interrupt to the given IRQ line
...
void send_eoi(uint32_t irq_num) {
    /* when sending EOI to slave, we send to IRQ line on slave PIC and slave IRQ line on master PIC */
